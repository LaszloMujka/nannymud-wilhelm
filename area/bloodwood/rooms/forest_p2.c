...
// -- Properties ---------------------------------------------------------
  ...
  add_property("forest");
  add_light(1);
  add_exit("east","forest_p1",0,"@check_npc()");
  add_exit("northwest", "forest_p3");
  add_hidden_exit("south", "forest5", "south, into the forest","check_exit");
...
// -- Items and Commands -------------------------------------------------
  ...
  add_item(({ "west" }), 
    "A small path leads northwest further into the forest. ");
  add_item(({ "east" }), 
...
check_exit() {
     ...
     (this_player()-> query_puzzle("wilhelm_bloodwood_quest")))
  {
    ...
    "/players/wilhelm/area/bloodwood/rooms/forest5");	  
  return;
  }	
